...
I de flesta fall innehåller Adobe Pass Authentication API ytterligare felinformation i svarstexten för att kunna tillhandahålla **kontext** om varför ett visst fel har inträffat och/eller möjliga lösningar på problemet automatiskt.  *I vissa specifika fall som omfattar autentiserings- eller utloggningsflöden kan dock Adobe Pass Authentication Services returnera ett svar från HTML eller en tom brödtext. Mer information finns i API-dokumentationen.*
...
|---|---|----|:---:|---|
| *status* | *heltal* | *403* | &amp;kontrollera; | Svarets HTTP-statuskod enligt RFC 7231 (<https://tools.ietf.org/html/rfc7231#section-6>) <ul><li>400 Ogiltig begäran</li><li>401 Obehörig</li><li>403 Förbjuden</li><li>404 Hittades inte</li><li>405 Metoden tillåts inte</li><li>409 Konflikt</li><li>410 borta</li><li>412 Förhandsvillkor misslyckades</li><li>429 För många begäranden</li><li>500-intervallserverfel</li><li>503 Tjänsten är inte tillgänglig</li></ul> |
| *kod* | *string* | *network_connection_error* | &amp;kontrollera; | Standardfelkoden för Adobe Pass-autentisering. En fullständig lista över felkoder finns nedan. |
...
| *åtgärd* | *string* | *försök igen* | &amp;kontrollera; | Rekommenderade åtgärder för att åtgärda situationen: <ul><li> *ingen* - Tyvärr finns det ingen fördefinierad åtgärd för att åtgärda problemet. Detta kan tyda på ett felaktigt anrop av det offentliga API:t</li><li>*konfiguration* - En konfigurationsändring krävs via TVE-kontrollpanelen eller genom att kontakta support. </li><li>*programregistrering* - Programmet måste registrera sig igen. </li><li>*autentisering* - Användaren måste autentisera eller återautentisera. </li><li>*auktorisation* - Användaren måste få behörighet för den specifika resursen. </li><li>*nedbrytning* - Någon form av nedbrytning bör appliceras. </li><li>*försök igen* - Om du försöker igen kanske problemet kan lösas</li><li>*försök igen efter* - Om du försöker göra om begäran efter den angivna tidsperioden kan problemet lösas.</li></ul> |
...
För den andra och tredje kategorin är den faktiska åtgärdsimplementeringen i hög grad beroende av programfunktionerna. Till exempel &quot;*nedbrytning*&quot; kan implementeras antingen som &quot;växla till 15 minuter temporära pass för att tillåta användare att spela upp innehållet&quot; eller antingen som &quot;automatiskt verktyg för att tillämpa FÖRSTÖRNINGEN AUTHN-ALL eller AUTHZ-ALL för integrering med det angivna MVPD&quot;. Liknande en &quot;*autentisering*&quot; kan utlösa en passiv autentisering (bakåtkanalsautentisering) på en surfplatta och ett helskärmsautentiseringsflöde på anslutna tv-apparater. Därför valde vi att tillhandahålla fullständiga URL:er med schema och alla parametrar.
...
| **programregistrering** | *access_token_expirate* | 401 | Åtkomsttoken har gått ut. Programmet bör uppdatera åtkomsttoken enligt API-dokumentationen. |
