...
>[!NOTE]
>
>MVPD-program kan tillhandahålla en eller flera slutpunkter för varje flöde. När du integrerar en ny kanal måste MVPD ange vilken slutpunkt de vill använda för varje flöde.

>[!IMPORTANT]
>
>Alla ändringar av slutpunkter påverkar det övergripande beteendet för en integrering. Dessa ändringar bör endast implementeras efter att en bekräftelse har tagits emot från MVPD.
...
>[!IMPORTANT]
>
> Inställningsarvet följer en kedja som börjar med MVPD-inställningarna (som är de vanligaste), och sedan MVPD slutpunkt, integrering, plattformskategori och plattform (som har det mest specifika värdet).
...
Egenskapen **Attempt HBA** för en viss MVPD kommer endast att aktiveras eller inaktiveras efter [granskning och push-ändringar](/help/authentication/user-guide-tve-dashboard/tve-dashboard-review-push-changes.md).
...
>[!NOTE]
>
>Vissa egenskaper kan inte tas bort eftersom de är obligatoriska krav för den valda MVPD.
...
>[!NOTE]
>
>Varje MVPD kan ha olika parametrar. Mer information om parametrarna som en viss MVPD kan dela får du från Adobe.
...
Så här skapar du en ny integrering med en ny MVPD i din nuvarande konfiguration:
...
1. Välj **Skapa ny integrering** längst upp till höger i avsnittet **Integrationer**.
   ...
   **Markera slutpunkter**
   ...
   När du har valt den MVPD som krävs fylls **Select-slutpunktsavsnittet** i automatiskt med de standardslutpunkter som är konfigurerade för just den MVPD.
   ...
   **Ytterligare information**
   ...
   Det här avsnittet innehåller olika egenskaper som måste konfigureras för den valda MVPD-filen i avsnittet **Markera kanal och MVPD**.

   >[!NOTE]
   >
   > De faktiska egenskaperna kan skilja sig åt beroende på vilka MVPD-program som har valts i avsnittet **Välj kanal och MVPD**.
...
När integreringen har inaktiverats förlorar slutanvändarna möjligheten att autentisera eller auktorisera med specifika MVPD.
