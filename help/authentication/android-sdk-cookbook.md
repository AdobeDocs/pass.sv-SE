...
1. Initiera Adobe Pass-autentisering
   ...
   a. Utlysning [`getInstance`](#$getInstance) för att skapa en instans av Adobe Pass Authentication AccessEnabler.

   - **Beroende:** Adobe Pass Authentication Native Android Library (AccessEnabler)

   b. Utlysning` setRequestor()` fastställa programmerarens identitet, skicka in programmerarens `requestorID` och (valfritt) en array med slutpunkter för Adobe Pass-autentisering.

   - **Beroende:** Giltigt Adobe Pass Authentication RequestorID\
